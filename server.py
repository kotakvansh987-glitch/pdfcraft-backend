...
# ── DOWNLOAD (fixed — works on all browsers + phones) ──
@app.route('/api/download/<path:did>')
def download(did):
    # Search for the file with any extension
    extensions = ['.pdf', '.zip', '.jpg', '.jpeg', '.png', '.webp', '.bmp', '.gif']
    found_path = None
    found_ext = ''
    for ext in extensions:
        ...
        if os.path.exists(fp):
            ...
            break
    if not found_path:
        # Try finding by prefix (in case extra chars)
        for fn in os.listdir(OUT):
            if fn.startswith(did.split('_')[0]):
                found_path = os.path.join(OUT, fn)
                found_ext = os.path.splitext(fn)[1]
                break
    if not found_path:
        return jsonify({'error': 'File not found or expired'}), 404

    mime = MIME.get(found_ext, 'application/octet-stream')
...
def compress():
    ...
    did = str(uuid.uuid4())
    outp = os.path.join(OUT, did + '_compressed.pdf')
    try:
        ...
        return jsonify({'success': True, 'download_id': did + '_compressed',
...
def split():
    ...
    did = str(uuid.uuid4())
    tmpdir = os.path.join(OUT, did + '_split_pages')
    ...
    return jsonify({'success': True, 'download_id': did + '_split',
                    'filename': 'split_pages.zip', 'pages_count': len(files_out)})
...
def pdf_to_img():
    ...
    inp = save_file(request.files['file'])
    did = str(uuid.uuid4())
    tmpdir = os.path.join(OUT, did + '_pdfimgs')
    os.makedirs(tmpdir, exist_ok=True)
    ...
    for i, page in enumerate(reader.pages):
        ...
        img.save(fp)
        files_out.append(fp)
    zip_path = os.path.join(OUT, did + '_pdfimgs.zip')
    with zipfile.ZipFile(zip_path, 'w') as zf:
        for fp in files_out: zf.write(fp, os.path.basename(fp))
    return jsonify({'success': True, 'download_id': did + '_pdfimgs',
                    'filename': 'pdf_pages.zip', 'pages_count': len(files_out)})
...
def compress_image():
    ...
    if mode == 'quality':
        if save_fmt == 'PNG':
            lvl = max(0, min(9, int((100-quality)/11)))
            img.save(outp, 'PNG', optimize=True, compress_level=lvl)
        else:
            img.save(outp, 'JPEG', quality=quality, optimize=True)

    elif mode == 'kb':
        import io
        target_bytes = target_kb * 1024
        lo, hi, best_q = 5, 95, 75
        best_buf = None
        for _ in range(12):
            mid = (lo + hi) // 2
            buf = io.BytesIO()
            ...
            sz = buf.tell()
            if sz <= target_bytes:
                best_q = mid; best_buf = buf; lo = mid + 1
            else:
                hi = mid - 1
        if best_buf:
            best_buf.seek(0)
            with open(outp, 'wb') as fout: fout.write(best_buf.read())
        else:
            img.save(outp, 'JPEG', quality=5, optimize=True)
        ...
        img.save(outp, 'JPEG', quality=dpi_quality, optimize=True, dpi=(dpi_val, dpi_val))

    new_size = os.path.getsize(outp)
    return jsonify({
        'success': True,
        'download_id': did + '_compressed',
        'filename': 'compressed' + out_ext,
...
def resize_image():
    ...
    return jsonify({'success': True, 'download_id': did + '_resized',
...
def convert_image():
    ...
    if len(files) == 1:
        ...
        return jsonify({'success': True, 'download_id': did + '_converted',
                        'filename': 'converted.' + out_fmt})
    else:
        tmpdir = os.path.join(OUT, did + '_convimgs')
        os.makedirs(tmpdir, exist_ok=True)
        for fi in files:
            ...
            img.save(outp, 'JPEG' if out_fmt in ['jpg','jpeg'] else out_fmt.upper())
        zip_path = os.path.join(OUT, did + '_convimgs.zip')
        with zipfile.ZipFile(zip_path, 'w') as zf:
            for fn in os.listdir(tmpdir): zf.write(os.path.join(tmpdir,fn), fn)
        return jsonify({'success': True, 'download_id': did + '_convimgs',
